...
import { AnalysisResult, formatPhoneNumber } from '@/types/mapper';

interface MappingViewProps {
    data: AnalysisResult;
    onBack: () => void;
    isEntering: boolean;
    onImport: (mappings: any[]) => void;
    importResult: any;
    isImporting: boolean;
}

const MappingView = ({ data, onBack, isEntering, onImport, importResult, isImporting }: MappingViewProps) => {
    ...
    const handleConfirmImport = () => {
        // Prepare mappings in the format expected by the API
        // API expects array of { sourceField, targetField, confidence ... }
        // We construct this from current 'mappings' state and original 'data.suggestedMapping'

        const params = Object.entries(mappings).map(([sourceCol, targetCol]) => {
            if (targetCol === 'skip') return null;

            // Find original confidence if target matches suggestion, else assume 1.0 (manual override)
            const originalSuggestion = data.suggestedMapping[sourceCol];
            const confidence = (originalSuggestion && originalSuggestion.targetField === targetCol)
                ? originalSuggestion.confidence
                : 1.0;
    ...
    return (
        <div
            className={`flex flex-col min-h-screen transition-all duration-700 ${isEntering ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-[30px]'
                }`}
        >
            {/* Header Bar */}
                                    ...
                                    <TableHeader>
                                        <TableRow className="bg-slate-50/80">
                                            {data.sourceColumns.map((col) => (
                                                <TableHead
                                                    key={col}
                                                    className={`font-medium text-foreground whitespace-nowrap transition-opacity duration-300 ${isColumnSkipped(col) ? 'opacity-40' : ''
                                                        }`}
                                                >
                                                    {col}
                                                </TableHead>
                                        ...
                                        {data.sampleData.map((row, rowIndex) => (
                                            <TableRow key={rowIndex}>
                                                {data.sourceColumns.map((col) => (
                                                    <TableCell
                                                        key={col}
                                                        className={`font-mono text-sm whitespace-nowrap transition-opacity duration-300 ${isColumnSkipped(col) ? 'opacity-40' : ''
                                                            }`}
                                                    >
                                                        {row[col] || '—'}
                                                    </TableCell>
                                    ...
                                    <TableHeader>
                                        <TableRow>
                                            {data.sourceColumns.map((col) => (
                                                <TableHead
                                                    key={col}
                                                    className={`p-2 min-w-[180px] transition-opacity duration-300 ${isColumnSkipped(col) ? 'opacity-40' : ''
                                                        }`}
                                                >
                                        ...
                                        {data.sampleData.map((row, rowIndex) => (
                                            <TableRow key={rowIndex}>
                                                {data.sourceColumns.map((col) => (
                                                    <TableCell
                                                        key={col}
                                                        className={`font-mono text-sm whitespace-nowrap transition-all duration-300 ${isColumnSkipped(col) ? 'opacity-40 line-through' : ''
                                                            }`}
                                                    >
                                                        {/* Render value only if mapped */}
                                                        {!isColumnSkipped(col) ? getTransformedValue(col, row[col]) : <span className="opacity-0">-</span>}
                                                    </TableCell>
                                                ))}
                                            </TableRow>
                                        ))}
