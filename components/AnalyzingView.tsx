...
const AnalyzingView = ({ fileName, rowCount, isExiting }: AnalyzingViewProps) => {
  ...
  return (
    <div 
      className={`flex flex-col items-center justify-center min-h-[calc(100vh-2rem)] px-4 transition-all duration-500 ${
    ...
    >
      ...
      {/* Spinner Container */}
      <div className="relative mb-8 animate-fade-slide-up">
        {/* Outer Ring */}
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          {progress >= 100 ? (
            <CheckCircle2 className="w-10 h-10 text-success animate-fade-in" />
          ) : (
      ...
      {/* Status Text */}
      <h2 
        className="text-xl font-semibold text-foreground mb-2 animate-fade-slide-up"
        style={{ animationDelay: '100ms' }}
      >
        {progress >= 100 ? 'Анализ завершён!' : STAGES[stageIndex]}
      </h2>

      {/* File Info */}
      <p 
        className="text-muted-foreground mb-8 animate-fade-slide-up"
        style={{ animationDelay: '150ms' }}
      >
        {fileName} ({rowCount} строк)
      </p>
